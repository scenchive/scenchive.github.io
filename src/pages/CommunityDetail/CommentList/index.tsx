...
const CommentList = (props:
  ...
  return (
    <>{props?.commentList?.map((commentEach, index) => commentEach.parentId === null && props?.commentList!==undefined
        ?
        <div key={index}>
          {commentEach?.deleted === true ?
            <CommentRow key={commentEach?.id + 'deletedComment'} isLast={index === props?.commentList.length - 1}>
            ...
            <CommentRow
              ...
              <div style={{ display: "flex", flexDirection: "row" }}>
                ...
                <CommentDetailArea>
                  ...
                  <MinorArea>
                    ...
                    <ReplyButton onClick={() => props?.setIsReplyOn(props?.isReplyOn !== commentEach?.id ? commentEach?.id : null)}>답글 쓰기</ReplyButton>
                  </MinorArea>

                </CommentDetailArea>
                {commentEach?.memberId === props?.userInformation?.userId ?
                  <DeleteButton onClick={() => props?.deleteComment(commentEach?.id)} src="/assets/icon/icon_delete_comment_x.svg" />
                  : null}
              </div>
              ...
              {props?.isReplyOn === commentEach?.id
                ? <WriteCommentArea style={{ width: "calc(100% - 30px)", marginLeft: '30px', marginTop: "20px", marginBottom: "10px" }}>
                  ...
                  />
                  <CommentButton onClick={() => props?.writeReplyComment()}>등록</CommentButton>
                </WriteCommentArea>
                : null
              }
            </CommentRow>
          ...
          {props?.commentList !== undefined && props?.commentList?.map((el2) => el2?.parentId !== null && el2?.parentId === commentEach?.id&& props?.commentList!==undefined ?
            el2.deleted === true ?
              <CommentRow key={el2?.id + 'deletedReply'} isLast={index === props?.commentList.length - 1}>
                ...
                <div style={{ display: "flex", flexDirection: "row" }}>
                  ...
                  <CommentDetailArea>
                    ...
                    <MinorArea>
                      ...
                      <ReplyButton onClick={() => props?.setIsReplyOn(props?.isReplyOn !== el2?.id ? el2?.id : null)}>답글 쓰기</ReplyButton>
                    </MinorArea>
                  </CommentDetailArea>
