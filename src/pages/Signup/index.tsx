...
import { FRAGRANCEWHEELKEYWORDSTYPE, KEYWORDTAGSTYPE, MOODKEYWORDSTYPE } from "../../common/types";
...
const Signup = () => {
  ...
  const { data: keyword, loading: keywordLoading, error: keywordError, fetchApi: fetchKeyword } = useApi<any>();
  const { data: signup, loading: signupLoading, error: signupError, fetchApi: fetchSignup } = useApi<any>();
  ...
  const signupAccount = async () => {
    ...
    const res = await fetchSignup('post', '/signup', formData, { 'Content-Type': 'multipart/form-data', accept: 'application/json' })
