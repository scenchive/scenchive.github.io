...
import { WriteReviewKeywordType, WriteReviewPerfumeDetailGroup } from "../../common/types";
import useApi from "../../hooks/useApi";

const WriteReview = () => {
  const navigate = useNavigate();
  const { data: checkToken, loading: checkTokenLoading, error: checkTokenError, fetchApi: fetchCheckToken } = useApi<any>();
  const { data: perfumeDetail, loading: perfumeDetailLoading, error: perfumeDetailError, fetchApi: fetchPerfumeDetail } = useApi<WriteReviewPerfumeDetailGroup>();
  const { data: keywordList, loading: keywordListLoading, error: keywordListError, fetchApi: fetchKeywordList } = useApi<any>();
  const { data: postReview, loading: postReviewLoading, error: postReviewError, fetchApi: fetchPostReview } = useApi<any>();
  ...
  const [fragranceWheelKeywords, setFragranceWheelKeywords] = useState<WriteReviewKeywordType[]>([]);
  ...
  const [keywordTagsArray, setKeywordTagsArray] = useState<WriteReviewKeywordType[]>([]);
  ...
  */
  ...
  useEffect(() => {
    if (keywordList !== undefined) {
      ...
      keywordList.map((el: WriteReviewKeywordType) => el.ptagtype_id === 1 ? fragranceWheelKeywordsArray.push(el) : el.ptagtype_id === 2 ? moodKeywordsArray.push(el) : placeArray.push(el))
